...
//
//   Revised        $Date: 2015-02-02 18:30:46 -0800 (Mon, 02 Feb 2015) $
//   Revision:      $Revision: 42252 $
//
...
uint16 NPITL_writeTL(uint8 *buf, uint16 len)
{
    ...
    // Length must be less than TX buffer size
    if(NPITL_checkNpiBusy()  || len > NPI_TL_BUF_SIZE)
    {
        ICall_leaveCriticalSection(key);
        return 0;
    }
