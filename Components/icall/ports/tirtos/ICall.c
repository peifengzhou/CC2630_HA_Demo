/**
  @file  ICall.c
  @brief Indirect function Call dispatcher implementaton on top of TI-RTOS.
  ...
  be overriden with a compile option.
 ...
 * The value may be overriden by a compile option.
 ...
 * One task is created per image to start off the image entry funciton.
 ...
 * Each initialization parameter (pointer) is passed to each correspnding
 ...
 * This simple heap depends on critial section implementation
...
{
  if (Hwi_create(args->intnum, (void (*)(UArg))args->isrfunc,
                 NULL, NULL) == NULL)
  {
    ICALL_HOOK_ABORT_FUNC();
    return ICALL_ERRNO_NO_RESOURCE;
...
static ICall_Errno ICall_primWaitMatch(ICall_WaitMatchArgs *args)
{
  ...
  /* Re-increment the concumed semaphores */
...
/**
 * @internal
 * Primitive service registerd handler function
 * @param args   arguments
 * @return error code
 */
