...
typedef struct _zstack_sysconfigwritereq_t
{
    ...
    /**
     * End Device Queued Poll Rate (not applicable for routers),
     * in milliseconds
     */
    ...
    /**
     * End Device Response Poll Rate (not applicable for routers),
     * in milliseconds
     */
...
typedef struct _zstack_sysconfigreadrsp_t
{
    ...
    /** Set to true if pollRate is available, false if not */
    bool has_pollRate;
    /** End Device Poll Rate (not applicable for routers) */
    uint16_t pollRate;
...
{
    /**
     * Duration of join. 0=disabled, 0xFF=enabled, 0x01-0xFE number seconds
     * to permit join
     */
    uint8_t duration;
...
typedef struct _zstack_apsfindallgroupsrsp_t
{
     ...
     * and must be deallocated [ICall_free(pGroupList)] by the receiving
     * application thread
     */
    uint16_t *pGroupList;
...
typedef struct _zstack_apsfindgrouprsp_t
{
     ...
     * [ICall_free(pName)] by the receiving application thread.
     */
    uint8_t *pName;
