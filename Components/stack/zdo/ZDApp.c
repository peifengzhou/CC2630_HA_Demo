/**************************************************************************************************
  Filename:       ZDApp.c
  Revised:        $Date: 2015-02-18 17:35:22 -0800 (Wed, 18 Feb 2015) $
  Revision:       $Revision: 42773 $
      ...
      if ( ZG_BUILD_JOINING_TYPE && ZG_DEVICE_JOINING_TYPE )
      {
        ...
        if ( ( (pChosenNwk = ZDApp_NwkDescListProcessing()) != NULL ) && (zdoDiscCounter > NUM_DISC_ATTEMPTS) )
        {
          ...
          else if ( devStartMode == MODE_REJOIN )
          {
            ...
            if ( ZG_SECURE_ENABLED && ( ZDApp_RestoreNwkKey( FALSE ) == TRUE ) )
            {
...
void ZDApp_LeaveReset( uint8 ra )
{
  ...
  if (ZG_DEVICE_ENDDEVICE_TYPE)
  {
    ...
    if ( ra == TRUE )
    {
      ...
      ZDApp_SavedPollRate = zgPollRate;
      zgResponsePollRate = RESPONSE_POLL_RATE;
      savedResponseRate = zgResponsePollRate;
      zgQueuedPollRate = QUEUED_POLL_RATE;
