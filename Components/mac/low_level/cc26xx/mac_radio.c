...
{
  (void) macGetRadioTxPowerReg(&txPower);
 ...
 *
 * @brief       Calculate transmitter power register value.
 *
 * @param       pTxPower - the minus dBm for power but as a postive integer (or if configured
 *                         for it, txPower is the raw register value). If PA/LNA is installed
 *                         then txPower becomes positive dBm.
 *
 * @return      tx_Power register setting
 **************************************************************************************************
 */
MAC_INTERNAL_API uint16 macGetRadioTxPowerReg(uint8 *pTxPower)
{
  halIntState_t  s;
  uint16 *pTable = ((macUserCfg_t *)macRadioConfig)->rfTxPowerTable;

  /* if the selected dBm is out of range, use the closest available */
  if ((int8)(*pTxPower) > (int8)pTable[MAC_RADIO_DEFS_TBL_TXPWR_FIRST_ENTRY])
  {
    /* greater than base value -- out of table range */
    *pTxPower = (uint8)pTable[MAC_RADIO_DEFS_TBL_TXPWR_FIRST_ENTRY];
  }
  else if ((int8)(*pTxPower) < (int8)pTable[MAC_RADIO_DEFS_TBL_TXPWR_LAST_ENTRY])
  {
    /* smaller than the lowest power level -- out of table range */
    *pTxPower = (uint8)pTable[MAC_RADIO_DEFS_TBL_TXPWR_LAST_ENTRY];
  }
   ...
   */
  {
    uint8 index = pTable[MAC_RADIO_DEFS_TBL_TXPWR_FIRST_ENTRY] - *pTxPower
...
MAC_INTERNAL_API void macRadioUpdateChannel(void)
{
  ...
  if (reqChannel != macPhyChannel)
  {
    ...
    DBG_PRINT1(DBGSYS, "MAC_RADIO_SET_CHANNEL(%u)", macPhyChannel);
...
uint8 macRadioEnergyDetectStop(void)
{
  ...
  DBG_PRINT1(DBGSYS, "ED RSSI = %u", energyDetectMeasurement);
  
  /* The radio is setup to do CMD_IEEE_ED_SCAN. 
   * Default radio to do CMD_IEEE_RX instead.
   */
  macSetupReceiveCmd();
...
static uint8 radioComputeED(int8 rssiDbm)
{
   ...
   */
  int8 sensitivity = ((macUserCfg_t *)macRadioConfig)->rfFeSensitivity;
  int8 saturation = ((macUserCfg_t *)macRadioConfig)->rfFeSaturation;
  
  min = ED_RF_POWER_MIN_DBM + sensitivity;
  max = ED_RF_POWER_MAX_DBM + saturation;
