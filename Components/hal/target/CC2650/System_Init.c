 ...
 *
 * @brief       This routine performs various system intialization.
 *
 * input parameters
 *
 * @param       None.
...
{
  // DCDC uses less power
  PowerCtrlSourceSet( PWRCTRL_PWRSRC_DCDC );

  // recharge controller settings
  //AONWUCRechargeCtrlConfigSet( TRUE, 34, 14816 );

  // Recommended numbers:
  // ADEN = 0x01
  // C2 = 0x04
  // C1 = 0x05
  // MAXRCPERIOD_M = 0x14 //max_recharge_period = 21440 32kHz cycles = 660ms
  // MAXRCPERIOD_E = 0x06
  // PERIOD_M = 0x1C //initial_recharge_period = 14816 32kHz cycles = 460ms
  // PERIOD_E = 0x05
  HWREG(AON_WUC_BASE + AON_WUC_O_RCCFG) =
       ((uint32_t)0x1 << AON_WUC_RCCFG_ADEN_S) |
       (0x04 << AON_WUC_RCCFG_C2_S) |
       (0x05 << AON_WUC_RCCFG_C1_S) |
       (0x14 << AON_WUC_RCCFG_MAXRCPERIOD_M_S) |
       (0x06 << AON_WUC_RCCFG_MAXRCPERIOD_E_S) |
       (0x1C << AON_WUC_RCCFG_PERIOD_M_S) |
       (0x05 << AON_WUC_RCCFG_PERIOD_E_S);
...
#ifdef POWER_SAVING
  // turn off JTAG when not in use
  HWREG(AON_WUC_BASE + AON_WUC_O_JTAGCFG) = 0;

#if !((defined(HAL_LCD)  && (HAL_LCD==TRUE))  ||   \
      (defined(HAL_LED)  && (HAL_LED==TRUE))  ||   \
      (defined(HAL_KEY)  && (HAL_KEY==TRUE))  ||   \
      (defined(HAL_UART) && (HAL_UART==TRUE)))
  AONIOCFreezeEnable();
#endif // !(HAL_LCD | HAL_LED | HAL_KEY | HAL_UART)
...
void System_Setup()
{
...
#if (defined(HAL_UART) && (HAL_UART==TRUE)) || (defined(HAL_LCD) && (HAL_LCD==TRUE))
...
#else // don't need UART or LCD (SPI)
  uint32_t powerMask = ( PRCM_SWPWR_CTRL_PERIPH |
                         PRCM_SWPWR_CTRL_RFCORE );
#endif // HAL_UART or HAL_LCD
  ...
  // [CTB] DMA not used by BLE application
  ...
  HWREG(PRCM_BASE + PRCM_O_SEC_DMA_RCG) = PRCM_SEC_DMA_RCG_TRNG | PRCM_SEC_DMA_RCG_CRYPTO;
  //HWREG(PRCM_BASE + PRCM_O_SEC_DMA_SCG) = PRCM_SEC_DMA_SCG_TRNG | PRCM_SEC_DMA_SCG_CRYPTO;
  //HWREG(PRCM_BASE + PRCM_O_SEC_DMA_DSCG) = PRCM_SEC_DMA_DSCG_TRNG | PRCM_SEC_DMA_DSCG_CRYPTO;
  ...
  // [CTB] Not used by BLE application
  ...
  while ((((HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_STATUS0 ))) & 1 ) == 0);
  // Switch safely to XOSC
...
void enableExternalClock( void )
{
 // Use external Clock: PC
  HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_AMPCOMP_TH2)           =  0x68880000; // Sets AMPCOMP settings for switching to XOSCHF
  HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_AMPCOMP_TH1)           =  0x886876A4; // Sets AMPCOMP settings for switching to XOSCHF
  HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_ANABYPASS_VALUE1)      =  0x24003FFF; // sets CAP SETTINGS
  HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_ANABYPASS_VALUE2)      =  0x780F0FFF; // Sets XOSCHF IBIAS THERM
  HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_XOSCLF_RCOSCLF_CTRL)   =  0x000000D8; // Sets default RCOSC_LF trim
  HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_ADC_DBLR_NANOAMP_CTRL) =  0x1F000000; // Sets default NANOAMP_BIAS settings
  HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_AMPCOMP_CTRL)          =  0x3000000; // Sets AMPCOMP settings for switching to XOSCHF
  HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_XOSCHF_CTRL)           =  0x00000040; // Sets XOSC in bypass
  HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_CTRL0)                 =  0x00010001; // Sets XOSC to clock source and allow switching
  // HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_CTRL0) =  0x00010005; // Sets XOSC to LF clock source and allow RCOSC_HF to be off
...
void prepareInternalClock( void )
{
  ...
  if (status0 & OSC_DIG_STATUS0_SCLK_HF_SRC_M)
  {
    ...
    while (((status0 = (HWREG(AUX_DDI0_OSC_BASE + OSC_DIG_O_STATUS0 ))) & 1 ) == 0);
...
//! Returns true if Iref override was performed; false if skipped becuase it
//! was previously done
//
//*****************************************************************************
static bool trimDevice(void)
{
    ...
    if(HWREGB(ADI3_BASE + ADI_3_REFSYS_O_ADI3_SPARE3) != FINAL_TRIM_DONE)
    {
        ...
        // Sets CAP SETTINGS
        DDI32RegWrite(AUX_DDI0_OSC_BASE, OSC_DIG_O_ANABYPASS_VALUE1, 0x000F0FFF);
        /*
        ui32Trim = (ui32Reg & FACTORY_CFG_CONFIG_OSC_TOP_XOSC_HF_CAP_TRIM_M) >>
            FACTORY_CFG_CONFIG_OSC_TOP_XOSC_HF_CAP_TRIM_S;
        DDI16BitfieldWrite(AUX_DDI0_OSC_BASE, OSC_DIG_O_ANABYPASS_VALUE1,
        ...
        */
        ...
        DDI16BitfieldWrite(AUX_DDI0_OSC_BASE, OSC_DIG_O_XOSCLF_RCOSCLF_CTRL,
                           OSC_DIG_XOSCLF_RCOSCLF_CTRL_RCOSCLF_CTUNE_TRIM_M,
                           OSC_DIG_XOSCLF_RCOSCLF_CTRL_RCOSCLF_CTUNE_TRIM_S,
                           ui32Trim);
        // Sets XOSCHF IBIAS THERM
        DDI32RegWrite(AUX_DDI0_OSC_BASE, OSC_DIG_O_ANABYPASS_VALUE2, 0x000003FF);
        // Sets AMPCOMP settings for switching to XOSCHF
        DDI32RegWrite(AUX_DDI0_OSC_BASE, OSC_DIG_O_AMPCOMP_TH2, 0x68880000);
        // Sets AMPCOMP settings for switching to XOSCHF
        DDI32RegWrite(AUX_DDI0_OSC_BASE, OSC_DIG_O_AMPCOMP_TH1, 0x886876A4);
        // Sets AMPCOMP settings for switching to XOSCHF
        DDI32RegWrite(AUX_DDI0_OSC_BASE, OSC_DIG_O_AMPCOMP_CTRL, 0x00713F27);
        // TBD : What does this do???
        DDI32RegWrite(AUX_DDI0_OSC_BASE, OSC_DIG_O_RADC_EXTERNAL_CFG, 0x403F4000);
         ...
         ADI_3_REFSYS_DCDC_CTRL_3_EN_DMY_3P3V))
    {
        ...
        // harmonic.
        //
        HWREGH(AUX_DDI0_OSC_BASE + DDI_O_SET + OSC_DIG_O_XOSCHF_CTRL) =
        ...
        // trick the frequency doubler in to locking to a lower harmonic.
        //
        HWREGB(AUX_ADI4_BASE + ADI_O_SET + ADI_4_AUX_O_CTL2_2_ADC_REF) =
            ADI_4_AUX_CTL2_2_ADC_REF_REF_IVREF_EN |
                ADI_4_AUX_CTL2_2_ADC_REF_REF_IREF_EN;
        HWREGB(AUX_ADI4_BASE + ADI_O_SET + ADI_4_AUX_O_CTL2_0_ADC) =
